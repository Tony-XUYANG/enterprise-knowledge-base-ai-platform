...
export function buildPasswordResetEmail(input: PasswordResetEmailInput) {
  ...
  return {
    subject: '重置你的 KnowledgeHub 密码',
    text: [
      `${input.displayName}，你好：`,
      '',
      `请在 ${input.expiresInMinutes} 分钟内打开以下链接设置新密码：`,
      input.resetUrl,
      '',
      '如果这不是你的操作，请忽略此邮件。',
    ].join('\n'),
  ...
  <body style="margin:0;background:#f4f7f6;font-family:Arial,'Microsoft YaHei',sans-serif;color:#18211f">
    <div style="max-width:560px;margin:0 auto;padding:40px 20px">
      <div style="background:#ffffff;border:1px solid #dbe3e0;padding:32px">
        ...
        <h1 style="font-size:22px;margin:0 0 16px">重置密码</h1>
        <p style="line-height:1.7;margin:0 0 12px">${displayName}，你好：</p>
        <p style="line-height:1.7;margin:0 0 24px">请在 ${input.expiresInMinutes} 分钟内设置新密码。</p>
        <a href="${resetUrl}" style="display:inline-block;background:#147d72;color:#ffffff;text-decoration:none;padding:11px 18px;border-radius:6px;font-weight:700">设置新密码</a>
