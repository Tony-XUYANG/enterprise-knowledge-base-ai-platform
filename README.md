...
管理员审计工作台汇总平台范围内的账号、登录、MFA、密码、会话、成员权限和邀请事件，支持时间范围、执行结果、事件类型、成员/操作人/邀请邮箱/IP 搜索及分页。每条记录区分操作人和影响账号，并展示设备、来源 IP 与发生时间；跨账号接口实时校验管理员权限，仍不会返回密码、令牌、API Key、MFA 密钥或请求正文。
