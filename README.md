...
应用工作台的调用日志按 24 小时、7 天、30 天或 90 天汇总外部 API 调用量、成功率、平均响应和 Token，并可按结果与访问密钥筛选。日志保留接口、HTTP 状态、稳定错误码、会话关联、密钥名称与安全前缀、来源 IP 和性能指标，不保存访问密钥、上游凭据或用户问题正文。

上线部署使用 `Dockerfile.api`、`Dockerfile.web` 和 `compose.production.yaml`；API 容器会在启动前自动重试并执行数据库迁移，生产配置会拒绝开发默认密钥和占位值。`/health/live` 与 `/health/ready` 分别用于存活和数据库就绪探针，优雅停机最多等待 8 秒。完整的备份、恢复、迁移和告警说明见 `docs/operations.md`。
